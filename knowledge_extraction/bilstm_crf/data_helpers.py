...
class NerDataProcessor(object):
    def __init__(self, max_len, vocab_size):
        ...
        self.word2id = {}  # 会把每一个字映射成为一个数字id

        self.tags = []  # 有哪些标签，例如B-disease
        self.tag2id = {}  # 把标签映射成数字
        self.id2tag = {}  # 把数字映射成标签

        self.class_nums = 0
        self.sample_nums = 0

    def read_data(self, path, is_training_data=True):  # 本函数的目的是：把字符串组合成一个序列样本和标签序列样本
        """
        数据格式如下（分隔符为空格）：
        便 B_disease
        秘 I_disease
        ...
        sentence = []
        labels = []
        split_pattern = re.compile(r'[；;。，、？！\.\?,! ]')  # 长句子尽量按照标点符号切分
        with open(path, 'r', encoding='utf8') as f:
            for line in f.readlines():
                # 每行为一个字符和其tag，中间用tab或者空格隔开
                line = line.strip().split()
                if(not line or len(line) < 2):  # 空行代表一句话结束
        ...
        if is_training_data:  # 如果是训练数据，初始化一些参数
