...
class CRF(Layer):
    ...
    def get_negative_log_likelihood(self, y_true, X, mask):
        ...
        if mask is not None:
            # modified !!! add '+1e-6'
            nloglik = nloglik / (K.sum(K.cast(mask, K.floatx()), 1) + 1e-6)
        else:
            nloglik = nloglik / \
                (K.cast(K.shape(X)[1], K.floatx()) + 1e-6)  # modified !!!
        return nloglik
    ...
    def recursion(self, input_energy, mask=None, go_backwards=False, return_sequences=True, return_logZ=True, input_length=None):
        ...
        if mask is not None:
            ...
            #     [tf.cast(mask,dtype=tf.float32), K.zeros_like(mask[:, :1])], axis=1), K.floatx())
