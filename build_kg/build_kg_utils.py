...
class MedicalExtractor(object):
    ...
    def extract_triples(self,data_path):
        ...
        with open(data_path,'r',encoding='utf8') as f:
            for line in tqdm(f.readlines(),ncols=80):
                data_json = json.loads(line) #将字符串转化为字典的格式
                disease_dict = {}#用于存放属性的字典
                ...
                if 'symptom' in data_json:
                    self.symptoms += data_json['symptom'] #把所有症状（是一个list）放到symtoms数组里
                    for symptom in data_json['symptom']:#然后处理每一个症状
                ...
                if 'cure_department' in data_json:
                    ...
                    if len(cure_department) == 2: #分情况处理
    ...
    def write_nodes(self,entitys,entity_type): #entity_type是实体的名称/标签，例如：药品
        print("写入 {0} 实体".format(entity_type))
        for node in tqdm(set(entitys),ncols=80):#利用set去重
    ...
    def set_attributes(self,entity_infos,etype): #只有疾病有属性，所以这是一个针对疾病实体的方法
        print("写入 {0} 实体的属性".format(etype))
        for e_dict in tqdm(entity_infos[892:],ncols=80):
            name = e_dict['name']
            del e_dict['name']
            for k,v in e_dict.items():
                if k in ['cure_department','cure_way']:#这两个属性都是列表，所以和其他属性写入方式不同
                    cql = """MATCH (n:{label})
                        WHERE n.name='{name}'
